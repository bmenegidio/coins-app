...
describe('CoinApiService', () => {
  ...
  it('should return normalized asset getById', async () => {
    jest
      .spyOn(httpService, 'get')
      .mockImplementationOnce(() => of(coinApiAssetsResponse));
    const asset = await firstValueFrom(service.getById('BTC'));
    expect(asset).toEqual(assetsNormalizedResults.at(0));
  });

  it('should return instance of NotFoundException when coinApiAssetsResponse is empty array', async () => {
    const coinApiAssetsResponse: AxiosResponse<CoinApiAssetsResponseRo> = {
      data: [],
    } as AxiosResponse;
    ...
    jest
      .spyOn(httpService, 'get')
      .mockImplementationOnce(() => of(coinApiAssetsResponse));
    await expect(firstValueFrom(service.getById('BTC'))).rejects.toThrow(
      NotFoundException
    );
  });
